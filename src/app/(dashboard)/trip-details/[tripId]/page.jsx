...
const TripDetails = observer(() => {
  ...
  const getTripDetails = async () => {
    try {
      ...
      if (trips.current_location) {
        markers.push({
          position: [
            trips.current_location.coordinates[1],
            trips.current_location.coordinates[0],
          ],
      ...
      if (trips.start_point) {
        markers.push({
          position: [
            trips.start_point.coordinates[1],
            trips.start_point.coordinates[0],
          ],
      ...
      if (trips.end_point) {
        markers.push({
          position: [
            trips.end_point.coordinates[1],
            trips.end_point.coordinates[0],
          ],
  ...
  useEffect(() => {
    if (mapMarkers.length > 0 && mapRef.current) {
      const bounds = L.latLngBounds(
        mapMarkers.map((marker) => marker.position)
      );
  ...
  return (
    <Box
      className="p-4"
      sx={{ backgroundColor: "transparent", minHeight: "100vh" }}
    >
      {/* A Map */}
      ...
      <div style={{ display: "flex", justifyContent: "end" }}>
        <Button
          variant="contained"
          color="primary"
          onClick={() => alert("Get Help with Trip")}
        >
          Get Help with Trip
        </Button>
      </div>
